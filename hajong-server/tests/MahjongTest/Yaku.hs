...
tests = testGroup "Standard Yaku"
    ...
    , testCase "San ankou NOT when discard is part of the third set" $ do
        let vi = valueInfo & vHand.handConcealed._Wrapped .~ ["M1", "M1", "M1", "S1", "S1", "S1", "W", "W"]
                           & vHand.handCalled .~ [fromShout $ Shout Chi Nan "S7" ["S8", "S9"], fromShout $ Shout Ron Nan "S2" ["S2", "S2"]]
                           & vHand.handPicks .~ [AgariCall $ Shout Ron Nan "S2" ["S2", "S2"]]
        snd (getYaku vi) @?= []
    ...
    , testCase "San Shoku Doukou" $ do
        let vi = valueInfo & vHand.handConcealed._Wrapped .~ ["S1", "S1", "S1", "P1", "P1", "P1", "W", "W"]
                           & vHand.handCalled .~ [fromShout $ Shout Pon Nan "M1" ["M1", "M1"], fromShout $ Shout Ron Nan "S3" ["S5", "S4"]]
                           & vHand.handPicks .~ [AgariCall $ Shout Ron Nan "S3" ["S5", "S4"]]
    ...
    , testCase "Shou Sangen + 2 x Yakuhai" $ do
        let vi = valueInfo & vHand.handConcealed._Wrapped .~ ["R!", "R!"]
                           & vHand.handCalled .~ [fromShout $ Shout Pon Nan "G!" ["G!", "G!"], fromShout $ Shout Pon Nan "W!" ["W!", "W!"], fromShout $ Shout Pon Nan "P5" ["P5", "P5"], fromShout $ Shout Ron Nan "M3" ["M1", "M2"]]
                           & vHand.handPicks .~ [AgariCall $ Shout Ron Nan "M3" ["M1", "M2"]]
    ...
    , testCase "Tanyao" $ do
        let vi = valueInfo & vHand.handConcealed._Wrapped .~ ["P2", "P3", "P4", "S5", "S6", "S7", "S2", "S3", "S4", "S8", "S8"]
                           & vHand.handCalled .~ [fromShout $ Shout Ron Nan "M2" ["M2", "M2"]]
                           & vHand.handPicks .~ [AgariCall $ Shout Ron Nan "M2" ["M2", "M2"]]
    ...
    , testCase "Chanta (open)" $ do
        let vi = valueInfo & vHand.handConcealed._Wrapped .~ ["S7", "S8", "S9", "M7", "M8", "M9", "W", "W"]
                           & vHand.handCalled .~ [fromShout $ Shout Chi Nan "P1" ["P2", "P3"], fromShout $ Shout Ron Nan "M1" ["M2", "M3"]]
                           & vHand.handPicks .~ [AgariCall $ Shout Ron Nan "M1" ["M2", "M3"]]
        snd (getYaku vi) @?= [Yaku 1 "Chanta"]

    , testCase "Kuitan" $ do
        let vi = valueInfo & vHand.handConcealed._Wrapped .~ ["S5", "S6", "S7", "S2", "S3", "S4", "S8", "S8"]
                           & vHand.handCalled .~ [fromShout $ Shout Chi Nan "P2" ["P3", "P4"], fromShout $ Shout Ron Nan "M2" ["M2", "M2"]]
                           & vHand.handPicks .~ [AgariCall $ Shout Ron Nan "M2" ["M2", "M2"]]
        snd (getYaku vi) @?= [Yaku 1 "Kuitan"]

    , testCase "Honitsu (open)" $ do
        let vi = valueInfo & vHand.handConcealed._Wrapped .~ ["M5", "M6", "M7", "M2", "M3", "M4", "W", "W"]
                           & vHand.handCalled .~ [fromShout $ Shout Chi Nan "M4" ["M5" , "M6"], fromShout $ Shout Ron Nan "M2" ["M2", "M2"]]
                           & vHand.handPicks .~ [AgariCall $ Shout Ron Nan "M2" ["M2", "M2"]]
    ...
    , testCase "Chiitoitsu" $ do
        let vi = valueInfo & vHand.handConcealed._Wrapped .~ ["M2", "M2", "M5", "M5", "M7", "M7", "P3", "P3", "M9", "M9", "M1", "M1"]
                           & vHand.handCalled .~ [fromShout $ Shout Ron Nan "M3" ["M3"]]
                           & vHand.handPicks .~ [AgariCall $ Shout Ron Nan "M3" ["M3"]]
    ...
    , testCase "Houtei Raoyui" $ do
        let vi = valueInfo & vHand.handConcealed._Wrapped .~ ["S5", "S6", "S7", "S2", "S3", "S4", "S8", "S8"]
                           & vHand.handCalled .~ [fromShout $ Shout Chi Nan "P2" ["P3", "P4"], fromShout $ Shout Ron Nan "M1" ["M1", "M1"]]
                           & vHand.handPicks .~ [AgariCall $ Shout Ron Nan "M1" ["M1", "M1"]]
    ...
    , testCase "Rinshan Kaihou" $ do
        let vi = valueInfo & vHand.handConcealed._Wrapped .~ ["M1", "M1", "S5", "S6", "S7", "S2", "S3", "S4", "S8", "S8"]
                           & vHand.handCalled .~ [fromShout $ Shout Chi Nan "P2" ["P3", "P4"]]
                           & vHand.handPicks .~ [AgariTsumoWanpai "M1"]
