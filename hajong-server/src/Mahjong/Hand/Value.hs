...
-- >>> let kyoku = Kyoku {_pRound = (Ton, 1), _pTurn = Ton, _pOja = Player 3, _pFirstOja = Player 3, _pWallTilesLeft = 70, _pDora = ["P4"], _pPlayers = error "not used", _pHonba = 0, _pRiichi = 0, _pResults = Nothing, _pDeals = [], _sEvents = [], _sHands = error "not used", _sWall = [], _sWanpai = Wanpai [] [] ["M1"] [], _sWaiting = Nothing}
...
getYaku vi | null possibleYakuCombos = (headEx groupings, [])
  ...
  where groupings          = filter complete $ getGroupings vi
        possibleYakuCombos = map (\grp -> (grp, checkYaku allStandard vi grp)) groupings
